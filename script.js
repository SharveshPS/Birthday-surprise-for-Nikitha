...
// ========== BACKGROUND MUSIC (works on iPhone / iPad / laptop) ==========
const BGM = {
    bgm1: null,
    bgm2: null,
    current: null,
    ...
    // Call this ONLY from a user tap
    unlockAndPlayBgm1() {
        this.init();
        if (this.unlocked && this.current === this.bgm1) return;

        this.bgm1.currentTime = 0;
        const p = this.bgm1.play();
        if (p && p.then) {
            p.then(() => {
                this.unlocked = true;
                this.current = this.bgm1;
                console.log('BGM1 playing');
            }).catch(err => {
                console.warn('BGM1 blocked:', err);
            });
        }
    },
    ...
    switchToBgm2() {
        ...
        this.bgm2.currentTime = 0;
        const p = this.bgm2.play();
        if (p && p.then) {
            p.then(() => {
                this.current = this.bgm2;
                console.log('BGM2 playing');
            }).catch(err => {
                console.warn('BGM2 blocked:', err);
            });
        }
    }
};

// Unlock and play audio on the very first user interaction anywhere on the page
document.addEventListener('click', function unlockAndPlayAudio() {
    // If your audio is a global variable (e.g., bgMusic or backgroundAudio)
    if (typeof bgMusic !== 'undefined' && bgMusic.paused) {
        bgMusic.play().catch(err => console.log("Audio play prevented:", err));
    } 
    // Or if you are querying an audio element by ID
...
// ========== TOUCH DRAG SUPPORT (iPad / phone) ==========
// Works with existing HTML5 drag-drop logic for Game 2 & Game 4

function enableTouchDrag(el) {
    if (!el || el.dataset.touchDrag === '1') return;
    el.dataset.touchDrag = '1';
    ...
    el.addEventListener('touchstart', (e) => {
        ...
        // Fire dragstart for existing handlers
        try {
            ...
            // Some browsers block synthetic DragEvent — backup below still works
        }
    }, { passive: true });
    ...
    el.addEventListener('touchend', (e) => {
        ...
        // ---- Game 2: drop card into timeline / reorder ----
        const dropZone = target.closest('#timeline-dropzone, #timeline-items');
        const targetCard = target.closest('.memory-card');

        if (el.classList.contains('memory-card') && (dropZone || targetCard)) {
            const zone = dropZone || targetCard.parentElement;
            if (targetCard && targetCard !== el) {
                zone.insertBefore(el, targetCard);
            } else if (zone) {
                zone.appendChild(el);
            }

            // Same side-effects as your existing drop handler
            const hint = document.getElementById('drop-hint');
            if (hint) hint.remove();

            const dropzone = document.getElementById('timeline-dropzone');
            if (dropzone && dropzone.querySelectorAll('.memory-card').length > 0) {
                dropzone.classList.add('has-cards');
            }
            const placed = dropzone ? dropzone.querySelectorAll('.memory-card').length : 0;
            if (placed >= 3) {
                const storyBtn = document.getElementById('story-btn');
                if (storyBtn) storyBtn.style.display = 'inline-block';
            }
        }

        // ---- Game 4: swap puzzle pieces ----
        const targetPiece = target.closest('.puzzle-piece');
        if (el.classList.contains('puzzle-piece') && targetPiece && targetPiece !== el) {
            const tempPos = el.style.backgroundPosition;
            el.style.backgroundPosition = targetPiece.style.backgroundPosition;
            targetPiece.style.backgroundPosition = tempPos;

            const tempCorrect = el.dataset.correct;
            el.dataset.correct = targetPiece.dataset.correct;
...
function enableTouchOnAllDraggables() {
    document.querySelectorAll('.memory-card[draggable="true"], .puzzle-piece[draggable="true"]')
        .forEach(enableTouchDrag);
}
...
function showVerification() {
    BGM.unlockAndPlayBgm1();   // ← real tap — works on iPhone/iPad
...
function initGame2() {
    ...
    enableTouchOnAllDraggables(); // ← add this line
}
...
const PUZZLE_IMAGE = "Images/Images/photof.jpeg";
...
const memoryPhotos = [
    "Images/Images/1.jpeg", "Images/Images/2.jpeg", "Images/Images/3.jpeg", "Images/Images/4.jpeg", "Images/Images/5.jpeg",
    "Images/Images/6.jpeg", "Images/Images/7.jpeg", "Images/Images/8.jpeg", "Images/Images/9.jpeg", "Images/Images/10.jpeg",
    "Images/Images/11.jpeg", "Images/Images/12.jpeg", "Images/Images/13.jpeg", "Images/Images/14.jpeg", "Images/Images/15.jpeg",
    "Images/Images/16.jpeg", "Images/Images/17.jpeg", "Images/Images/18.jpeg", "Images/Images/19.jpeg", "Images/Images/20.jpeg",
    "Images/Images/21.jpeg", "Images/Images/22.jpeg", "Images/Images/23.jpeg", "Images/Images/24.jpeg", "Images/Images/25.jpeg",
    "Images/Images/26.jpeg", "Images/Images/27.jpeg", "Images/Images/28.jpeg", "Images/Images/29.jpeg", "Images/Images/30.jpeg",
    "Images/Images/31.jpeg", "Images/Images/32.jpeg", "Images/Images/33.jpeg", "Images/Images/34.jpeg", "Images/Images/35.jpeg",
    "Images/Images/36.jpeg", "Images/Images/37.jpeg", "Images/Images/38.jpeg", "Images/Images/39.jpeg", "Images/Images/40.jpeg",
    "Images/Images/41.jpeg", "Images/Images/42.jpeg", "Images/Images/43.jpeg", "Images/Images/44.jpeg", "Images/Images/45.jpeg",
    "Images/Images/46.jpeg", "Images/Images/47.jpeg", "Images/Images/48.jpeg", "Images/Images/49.jpeg", "Images/Images/50.jpeg",
    "Images/Images/51.jpeg", "Images/Images/52.jpeg", "Images/Images/53.jpeg", "Images/Images/54.jpeg", "Images/Images/55.jpeg",
    "Images/Images/56.jpeg", "Images/Images/57.jpeg", "Images/Images/58.jpeg", "Images/Images/59.jpeg", "Images/Images/60.jpeg",
    "Images/Images/61.jpeg", "Images/Images/62.jpeg", "Images/Images/photof.jpeg"
];
